...
use crate::types::{Expression, Rows, Table, Value};

/// A table scan source.
pub fn scan(txn: &impl Transaction, table: Table, filter: Option<Expression>) -> RaftDBResult<Rows> {
    Ok(Box::new(txn.scan(&table.name, filter)?))
}

/// A primary key lookup source.
pub fn lookup_key(txn: &impl Transaction, table: String, keys: Vec<Value>) -> RaftDBResult<Rows> {
