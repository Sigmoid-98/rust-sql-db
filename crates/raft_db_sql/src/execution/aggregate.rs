...
use crate::types::{Expression, Row, Rows, Value};
...
impl Aggregator {
    ...
    fn add(&mut self, row: Row) -> RaftDBResult<()> {
    ...
    fn into_rows(self) -> RaftDBResult<Rows> {
...
impl Accumulator {
    ...
    fn add(&mut self, value: Value) -> RaftDBResult<()> {
    ...
    fn value(self) -> RaftDBResult<Value> {
        Ok(match self {
