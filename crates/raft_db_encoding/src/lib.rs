...
pub trait Key<'de>: Serialize + Deserialize<'de> {
    ...
    fn decode(bytes: &'de [u8]) -> RaftDBResult<Self> {
...
pub trait Value: Serialize + DeserializeOwned {
    ...
    fn decode(bytes: &[u8]) -> RaftDBResult<Self> {
        bincode::deserialize(bytes)
    }

    /// Decodes a value from a reader using Bincode.
    fn decode_from<R: Read>(reader: R) -> RaftDBResult<Option<Self>> {
        bincode::deserialize_from(reader)
    }

    /// Decodes a value from a reader using Bincode, or returns None if the
    /// reader is closed.
    fn maybe_decode_from<R: Read>(reader: R) -> RaftDBResult<Option<Self>> {
    ...
    fn encode_into<W: Write>(&self, writer: W) -> RaftDBResult<()> {
